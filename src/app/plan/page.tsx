...
export default function Home() {
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const res = await fetch("/api/itineraries", {//save an itinerary to the database
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          title: `Trip to ${destinations.map((d) => d.label.split(",")[0]).join(", ")}`,
      ...
      else setError(data.error ?? "Failed to save itinerary.");
    } catch {
    ...
    <div className="flex flex-col h-screen bg-background text-foreground">
      ...
      <header className="shrink-0 border-b border-border/40 flex items-center justify-between px-6 py-3">
        ...
        <div className="flex items-center gap-3">
          {session ? (
            // If the user is logged in, show their email and a link to the profile page
            <>
              <Link
                href="/profile"
              ...
              <div className="flex items-center gap-2">
                {session && (
                  <Button
                    ...
                    className="text-xs bg-[oklch(0.72_0.18_50)] text-[oklch(0.15_0_0)] hover:bg-[oklch(0.66_0.18_50)]"
                  >
                    {saved ? "Saved!" : saving ? "Saving…" : "Save Itinerary"}{/* Save itinerary button (states) */}
                  </Button>
                )}
